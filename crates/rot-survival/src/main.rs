...
fn main() {
    if let Err(error) = run() {
        ...
        eprintln!("{}", serde_json::to_string(&payload).expect("error payload must serialize"));
...
fn read_state(root: &Path) -> Result<SurvivalState, CliError> {
    ...
    assert_fresh(&seal, &seal).map_err(|error| {
        CliError::new(
            error.code,
            format!("canonical project state freshness fields are invalid: {}", error.message),
            11,
        )
    })?;
    Ok(state)
...
fn validate_checkpoint_identity(
    ...
    let slug = checkpoint_id.strip_prefix(CHECKPOINT_PREFIX).ok_or_else(|| {
        CliError::new(
    ...
    if checkpoint.get("observed_source_sha").and_then(Value::as_str)
