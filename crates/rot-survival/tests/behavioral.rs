use rot_survival::{FreshnessSeal, SurvivalEvent, assert_fresh, reduce_events};
...
fn fixture() -> Value {
    ...
    serde_json::from_str(
        &fs::read_to_string(root.join("fixtures/golden/survival-behavior-v2.json")).unwrap(),
    )
    .unwrap()
}
        ...
        match expected["status"].as_str().unwrap() {
            "PASS" => {
                let result = outcome.unwrap_or_else(|error| {
                    panic!("{} unexpectedly rejected: {error}", vector["name"])
                });
                let checks: BTreeMap<String, Value> =
